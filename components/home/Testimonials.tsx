...
export default function Testimonials() {
  return (
    <section
      id="reviews"
      className="bg-[#3F4630] px-6 py-24 lg:px-10"
    >
      <div className="mx-auto grid max-w-[1400px] items-center gap-14 lg:grid-cols-[1.3fr_0.7fr]">
        <div>
          <h2 className="mb-10 font-[var(--font-heading)] text-4xl text-[#F8F3EC] md:text-5xl">
            Loved by Many
          </h2>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {testimonials.map((t) => (
              <div
                key={t.name}
                className="rounded-2xl bg-[#F5E6D3] p-6 shadow-lg"
              >
                <div className="mb-3 flex gap-1 text-[#E8B84B]">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <StarIcon key={i} className="h-4 w-4" />
                  ))}
                </div>
                <p className="text-sm leading-6 text-[#5A2D2F]">
                  {t.quote}
                </p>
                <p className="mt-4 text-xs font-medium uppercase tracking-wide text-[#8B9874]">
                  — {t.name}
                </p>
              </div>
            ))}
        ...
        <div className="relative mx-auto hidden max-w-xs rotate-[3deg] rounded-lg bg-white p-3 shadow-2xl lg:block">
          ...
          />
          <div className="absolute -bottom-4 -right-4 rotate-[-4deg] rounded-md bg-[#F8F3EC] px-4 py-2 shadow-lg">
            <p className="font-[var(--font-heading)] text-[#5A2D2F]">
              Thank you! ♥
            </p>
          </div>
        </div>
